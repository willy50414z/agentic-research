...
def init_schema(db_url: str | None = None) -> None:
    """Drop and recreate all tables from schema.sql. Destroys existing data."""
    sql = _SCHEMA_PATH.read_text(encoding="utf-8")
    with get_connection(db_url) as conn:
        with conn.cursor() as cur:
            cur.execute(sql)
    logger.info("Database schema initialised from %s.", _SCHEMA_PATH)
...
def get_project(project_id: str, db_url: str | None = None) -> dict | None:
    with get_connection(db_url) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, name, plugin_name, goal, config, workflow_step, created_at"
    ...
    return {
        ...
        "goal": row[3], "config": row[4], "workflow_step": row[5], "created_at": row[6],
    }
...
def get_workflow_step(project_id: str, db_url: str | None = None) -> str | None:
    """Return the current workflow_step for a project, or None if not set."""
    with get_connection(db_url) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT workflow_step FROM projects WHERE id = %s",
                (project_id,),
            )
            row = cur.fetchone()
...
def set_workflow_step(project_id: str, step: str | None, db_url: str | None = None) -> None:
    """Update the workflow_step for a project."""
    with get_connection(db_url) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE projects SET workflow_step = %s WHERE id = %s",
                (step, project_id),
            )
    logger.debug("workflow_step='%s' set for project '%s'.", step, project_id)


def merge_config(project_id: str, patch: dict, db_url: str | None = None) -> None:
    """Merge a partial dict into projects.config JSONB."""
    with get_connection(db_url) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE projects SET config = config || %s::jsonb WHERE id = %s",
                (json.dumps(patch), project_id),
            )
